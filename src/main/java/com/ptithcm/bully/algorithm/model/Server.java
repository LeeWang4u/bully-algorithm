...
public class Server {
    ...
    public void initListNode() {
        try {
            Node n = new Node(3000, "10.252.7.157", 0, false);
            Node n1 = new Node(3000, "10.252.7.206", 1, false);
            Node n2 = new Node(3000, "192.168.1.28", 2, false);
            Node n3 = new Node(3000, "192.168.1.33", 3, false);
...
// --- Helper methods for handling different message types ---
    ...
    private void handleTransfersMessage(String[] list, DataOutputStream writer) throws IOException {
        writer.writeUTF("coordinator has received transfer request");
    ...
    private void handleConfirmTransfersMessage() {
        tpSetMessage(tpChatbox, getCurrentTime() + ": Transfer successful!", 1);
...
// --- Helper methods for specific logic ---
    ...
    private void forwardMessageToSmallerIdNodes(String[] list, DataOutputStream writer) {
        for (Node i : listNode) {
            if (i.getId() < id && i.getId() != Integer.parseInt(list[1])) {
                try (Socket msgSocket = new Socket(i.getHost(), i.getPort());
    ...
    private void forwardElectionAnswer(String senderId) {
        try (Socket sk = new Socket(getNodeById(senderId).getHost(), getNodeById(senderId).getPort());
